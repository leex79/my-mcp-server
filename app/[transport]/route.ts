    ...
    (server) => {
        ...
        server.registerTool(
            ...
            async ({ timezone }) => {
                ...
                return { content: [{ type: 'text' as const, text: `현재 시각 (${timezone}): ${text}` }] }
            }
        )
        ...
        server.registerTool(
            'geocode',
            {
                ...
                inputSchema: {
                    ...
                    language: z.string().optional().default('ko').describe('결과 언어 (기본값: ko)'),
                    count: z.number().int().min(1).max(10).optional().default(1).describe('반환할 결과 수 (기본값: 1, 최대 10)')
                }
            },
            async ({ name, language, count }) => {
                try {
                    ...
                    if (!res.ok) {
                        return { content: [{ type: 'text' as const, text: `오류: 지오코딩 API 호출 실패 (HTTP ${res.status})` }] }
                    }
                    ...
                    if (!data.results || data.results.length === 0) {
                        return { content: [{ type: 'text' as const, text: `"${name}"에 해당하는 도시를 찾을 수 없습니다.` }] }
                    }
                    ...
                    return { content: [{ type: 'text' as const, text: `오류: 네트워크 요청 실패 — ${e instanceof Error ? e.message : String(e)}` }] }
                }
            }
        )
        ...
        server.registerTool(
            ...
            async ({ latitude, longitude }) => {
                try {
                    ...
                    const res = await fetch(url)
                    if (!res.ok) {
                        return { content: [{ type: 'text' as const, text: `오류: 날씨 API 호출 실패 (HTTP ${res.status})` }] }
                    }
                    ...
                    const text = [
                        ...
                        `날씨: ${describeWeatherCode(c.weather_code)} (${c.is_day ? '낮' : '밤'})`,
        ...
        server.registerTool(
            'generate-image',
            {
                description: 'HuggingFace FLUX.1-schnell 모델로 이미지를 생성합니다. x-hf-token 헤더가 필요합니다.',
                inputSchema: {
            ...
            async ({ prompt, num_inference_steps }, extra) => {
                ...
                const raw = headers['x-hf-token']
                const hfToken = Array.isArray(raw) ? raw[0] : raw
                if (!hfToken) {
                    return { isError: true, content: [{ type: 'text' as const, text: '오류: x-hf-token 헤더가 필요합니다.' }] }
                }
                try {
                    ...
                    return { content: [{ type: 'image' as const, data, mimeType: blob.type || 'image/png' }] }
                } catch (e) {
                    return { isError: true, content: [{ type: 'text' as const, text: `오류: 이미지 생성 실패 — ${e instanceof Error ? e.message : String(e)}` }] }
                }
            }
        )
            ...
            async (uri) => {
                const info = {
                    ...
                    tools: [
                        ...
                        { name: 'calculate', description: '두 숫자와 연산자를 입력하면 사칙연산 결과를 반환합니다.' },
                        { name: 'time', description: '현재 날짜와 시간을 반환합니다.' },
                        { name: 'geocode', description: '도시 이름을 입력하면 위도와 경도를 반환합니다.' },
                        { name: 'weather', description: '위도와 경도를 입력하면 현재 날씨 정보를 반환합니다.' },
                        { name: 'generate-image', description: 'HuggingFace FLUX.1-schnell 모델로 이미지를 생성합니다. x-hf-token 헤더 필요.' }
                    ],
                    resources: [{ uri: 'server://info', description: 'MCP 서버 메타데이터' }],
